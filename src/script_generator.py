...
class ScriptGenerator:
    ...
    def generate_national_weather_overview(self, weather_data):
        ...
        try:
            ...
            # 天気概況文を生成
            if pattern_texts:
                overview_text = "今日の天気は、" + "、".join(pattern_texts) + f"となっています{self._get_polite_ending()[:-1]}。"
                ...
                return "今日の天気は地域によって変化があります。お出かけの際は最新の天気予報をご確認ください。"
                
        except Exception as e:
            print(f"Error generating national weather overview: {e}")
            return "今日の天気は地域によって様々です。お出かけの際は最新の天気予報をご確認ください。"
...
if __name__ == "__main__":
    ...
    # 天気データを取得
    collector = weather_data.WeatherDataCollector()
